...
def init_dexined(model_folder):
    """load the DexiNed model feature"""
    # Create directory if it doesn't exist
    if not os.path.exists(model_folder):
        os.makedirs(model_folder)
        
    if not os.path.exists(os.path.join(model_folder, 'dexined.weight')):
        # Google Drive weight file URL
        weight_url = "https://drive.usercontent.google.com/download?id=1V56vGTsu7GYiQouCIKvTWl5UKCZ6yCNu&export=download&authuser=0&confirm=t&uuid=aec7796b-0ceb-49f7-a088-c4bf570e88b5&at=AN8xHoplUMo8RNrttyjOhhDwEMD8%3A1751368552822"
        weight_path = os.path.join(model_folder, 'dexined.weight')
        
        try:
            print(f"Downloading DexiNed weight file from Google Drive...")
            ...
            print(f"\nDexiNed weight file downloaded and saved successfully")
            
        except Exception as e:
            print(f"Error downloading DexiNed weights: {e}")
            if os.path.exists(weight_path):
                os.remove(weight_path)  # 불완전한 파일 삭제
...
if __name__ == '__main__':
    ...
    device = "cpu"
    img_path = "datasets/dlp_gan_dataset/Harvard/harvard_0.jpg"
    img = Image.open(img_path).convert("RGB")
    img = img.resize((img_width, img_height))
    img_np = np.array(img).astype(np.float32)
    img_np = np.transpose(img_np, (2, 0, 1))  # HWC to CHW
    input = torch.from_numpy(img_np).unsqueeze(0).repeat(batch_size, 1, 1, 1).to(device)
    ...
    init_dexined("./weights")
    model.load_state_dict(torch.load("./weights/dexined.weight", map_location=device))
    # Initialize DexiNed weights if needed
